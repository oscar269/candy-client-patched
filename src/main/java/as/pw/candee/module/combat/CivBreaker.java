     ...
     extends Module
 {
             ...
             if (this.placedCrystal && !this.brokeBlock && this.breakTimer.passedX(this.breakDelay.getValue())) {
                 ...
                 if (!breaking) {
                     if (!this.instantBreak.getValue()) {
                         ...
                         mc.player.connection.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.START_DESTROY_BLOCK, this.base, EnumFacing.DOWN));
                         mc.player.connection.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, this.base, EnumFacing.DOWN));
                     } else {
                         if (!this.noSwingBlock.getValue()) mc.player.swingArm(EnumHand.MAIN_HAND);
     ...
     private void tryPlaceCrystalImmediate() {
         if (this.placedCrystal)
             return;    if (!CrystalUtil.canPlaceCrystal(this.base))
             return;
         if (this.crystalSlot != 999) setItem(this.crystalSlot);
         EnumHand hand = (this.crystalSlot != 999) ? EnumHand.MAIN_HAND : EnumHand.OFF_HAND;
         mc.playerController.updateController();
     ...
     public void onRender3D() {
         try {
             if (isNull(target) || isNull(this.base))
                 return;    RenderUtil3D.drawBox(this.base, 1.0D, this.blockColor.getValue(), 63);
             if (this.outline.getValue()) RenderUtil3D.drawBoundingBox(this.base, 1.0D, this.width.getValue(), this.blockColor.getValue());
     ...
     public boolean findSpace(EntityPlayer player) {
         ...
         for (BlockPos offset : offsets) {
             ...
             if ((BlockUtil.getBlock(basePos) == Blocks.OBSIDIAN || BlockUtil.getBlock(basePos) == Blocks.BEDROCK) && (
                 BlockUtil.canPlaceBlockFuture(basePos.up()) || BlockUtil.getBlock(basePos.up()) == Blocks.OBSIDIAN) &&
                 CrystalUtil.canPlaceCrystal(basePos.up())) {
     ...
     public static boolean breaking = false; public final Setting<Float> preDelay; public final Setting<Float> crystalDelay; public final Setting<Float> breakDelay; public final Setting<Float> attackDelay; public final Setting<Float> endDelay; public final Setting<Float> range; public final Setting<Boolean> tick; public final Setting<Boolean> toggle; public final Setting<Boolean> noSwingBlock;
     public void placeBlock(BlockPos pos, Boolean packet) {
         BlockUtil.placeBlock(pos, packet);
         if (!this.noSwingBlock.getValue()) mc.player.swingArm(EnumHand.MAIN_HAND);
     }
     public final Setting<Boolean> packetPlace; public final Setting<Boolean> packetCrystal; public final Setting<Boolean> instantBreak; public final Setting<Boolean> toggleSilentPickel; public final Setting<Boolean> packetBreak; public final Setting<Boolean> offHandBreak; public final Setting<Boolean> skip; public final Setting<Integer> breakAttempts; public final Setting<Float> targetRange;
     public final Setting<Target> targetType;
     public static EntityPlayer target = null; public final Setting<Color> blockColor; public final Setting<Boolean> outline; public final Setting<Float> width; public BlockPos base; public BlockPos old; public boolean builtTrap; public boolean placedCrystal; public boolean brokeBlock; public boolean attackedCrystal; public boolean done; public int crystalSlot; public int obbySlot; public int pickelSlot; public int attempts; public Timer blockTimer; public Timer crystalTimer; public Timer breakTimer;
