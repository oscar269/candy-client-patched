 ...
 public class CrystalUtil
     ...
     public static Boolean getArmourFucker(EntityPlayer player, float percent) {
         for (ItemStack stack : player.getArmorInventoryList()) {
             ...
             float f = (float) (stack.getMaxDamage() - stack.getItemDamage()) / stack.getMaxDamage() * 100.0F;
         }
     ...
     public static List<BlockPos> possiblePlacePositions(float placeRange, boolean specialEntityCheck, boolean oneDot15) {
         ...
         List<BlockPos> filtered = getSphere(PlayerUtil.getPlayerPos(), placeRange, (int)placeRange, false, true, 0).stream().filter(pos -> (mc.world.getBlockState(pos).getBlock() != Blocks.AIR)).filter(pos -> canPlaceCrystal(pos, specialEntityCheck, oneDot15)).collect(Collectors.toList());
     ...
     public static boolean hasCrystal(BlockPos pos) {
         ...
         for (Entity crystal : crystals) {
             ...
             if (pos.equals(crystalPos.add(0, -1, 0))) {
                 return true;
             }
         }
         return false;
